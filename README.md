...
Default weights from [`config/weights_default.yaml`](config/weights_default.yaml) (v2.0 — EVA-disc PSR access replaces coupling at the same 0.17 weight slot): illumination 0.17, ice 0.17, eva_psr_access 0.17, los_to_earth 0.14, slope 0.12, thermal 0.07, hazard 0.06, seismic 0.10. **v2.0 is a methodology swap, not a re-weighting** — the slot value matches the v1.8 coupling slot 1:1 so any catalog-level shift attributable to the change reflects the new criterion's geometry, not a moved weight. **The 0.10 seismic weight reflects that scarp distance is one of multiple safety factors and is most relevant on long habitat-occupation timescales**; the other seven weights are scaled by 0.928 from the v1.5 vector so the simplex sums to 1.00 without changing relative ratios. **The 0.15 LOS weight (now 0.14 after seismic activation) was chosen *before* the v1.1.0 validation rerun on physics-and-operations grounds**: LOS is a real physical criterion NASA prioritises in mission planning, but more binary-ish than continuous, so weighted slightly less than illumination/ice/eva_psr_access. Older weight vectors are preserved: [`config/weights_legacy.yaml`](config/weights_legacy.yaml) (5-criterion baseline, no coupling/LOS), [`config/weights_legacy_v6.yaml`](config/weights_legacy_v6.yaml) (v1.0.0 6-criterion, no LOS); the v1.8 vector is the v2.0 vector with `eva_psr_access` renamed back to `coupling`.
