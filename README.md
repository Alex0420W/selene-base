...
| **Multi-volatile** (v2.1; replaces Ice) | $s = \frac{1}{3}(s_{\text{H}_2\text{O}} + s_{\text{CO}_2/\text{NH}_3} + s_{\text{ultra}})$ where each $s_{\text{class}}$ = fraction of 2 km EVA-disc cells with `temp_max < T_{\text{class}}` | **Diviner PRP** `temp_max` (PDS4) | 240 m | circular-kernel convolution | $r_{\text{EVA}} = 2\,$km; $T_{\text{H}_2\text{O}} < 110\,$K, $T_{\text{CO}_2/\text{NH}_3} < 66\,$K, $T_{\text{ultra}} < 60\,$K |
...
| **PGV-seismic** (v2.2; replaces Seismic v1.8) | $s = 1 / (1 + \exp((\sum_s e^{-d_s/L} - m)/\sigma))$ on cumulative PGV from every scarp $s$ within cutoff | Mishra & Kumar 2022 (bundled, primary) + Watters 2015 polar (bundled, attribution anchor) | vector -> 240 m kernel | KDTree, 1 km densified vertices, per-scarp-min bucketing | $L=50\,$km, cutoff $=5L=250\,$km, $m=0.45$, $\sigma=0.20$ |
...
Default weights from [`config/weights_default.yaml`](config/weights_default.yaml) (v2.2 — PGV-seismic kernel replaces distance-to-nearest-scarp at the same 0.10 weight slot): illumination 0.17, multi_volatile 0.17, eva_psr_access 0.17, los_to_earth 0.14, slope 0.12, thermal 0.07, hazard 0.06, pgv_seismic 0.10. **All three v2.x swaps (v2.0 coupling → eva_psr_access; v2.1 ice → multi_volatile; v2.2 seismic → pgv_seismic) are methodology swaps, not re-weightings** — every new criterion sits in the slot its v1.x predecessor held, so any catalog-level shift reflects the new criterion's geometry, not a moved weight on the simplex. Inside multi_volatile, the three sub-classes (H₂O, CO₂/NH₃, ultra-cold) are equally weighted by default (1/3 each) — agnostic on NASA's preference ordering between volatile classes; expose `sub_weights=` for downstream sensitivity. Inside pgv_seismic, the sigmoid is anchored at `cum_pgv = exp(-40 km / 50 km) ≈ 0.45` (Watters 2024 single-strong-shaking-event PGV) → score 0.5; relax this calibration via the `sigmoid_midpoint` / `sigmoid_scale` parameters if a relative-rank-within-polygon signal is wanted instead. **The 0.10 pgv_seismic weight reflects that seismic risk is one of multiple safety factors and is most relevant on long habitat-occupation timescales**; the other seven weights are scaled by 0.928 from the v1.5 vector so the simplex sums to 1.00 without changing relative ratios. **The 0.15 LOS weight (now 0.14 after seismic activation) was chosen *before* the v1.1.0 validation rerun on physics-and-operations grounds**: LOS is a real physical criterion NASA prioritises in mission planning, but more binary-ish than continuous, so weighted slightly less than illumination/multi_volatile/eva_psr_access. Older weight vectors are preserved: [`config/weights_legacy.yaml`](config/weights_legacy.yaml) (5-criterion baseline, no coupling/LOS), [`config/weights_legacy_v6.yaml`](config/weights_legacy_v6.yaml) (v1.0.0 6-criterion, no LOS); the v1.8 vector is the v2.2 vector with the three v2.x criteria renamed back to their v1.x predecessors (eva_psr_access → coupling, multi_volatile → ice, pgv_seismic → seismic).
...
| PGV-seismic (cumulative shaking; v2.2; replaces Seismic v1.8) | Site Availability + Crew Safety | Mishra & Kumar 2022 + Watters 2015 (consistent; bundled in-repo since v1.8) | strong-shaking distance documented (Watters 2024 PSJ Mw 5.3 at 40 km); per-cell PGV thresholds not public |
...
- **v2.2 — PGV-style seismic shaking attenuation kernel (NASA-alignment Gap #6).** ✅ Replaces v1.8's distance-to-nearest-scarp seismic criterion (logistic on the single closest scarp; midpoint 25 km, steepness 8 km) with a **per-cell cumulative PGV (peak ground velocity) attenuation kernel** that aggregates contributions from *every* mapped scarp within a cutoff radius: `cum_pgv(cell) = Σ_{scarp s within 5L} exp(-d_s / L)` with `L = 50 km` (Watters 2024 PSJ Mw 5.3 strong-shaking distance) and cutoff `5L = 250 km` (captures > 99 % of any single scarp's contribution). The combined score is a sigmoid `1 / (1 + exp((cum_pgv − m) / σ))` anchored to the single-strong-shaking-event PGV (`m = 0.45 ≈ exp(-40/50)`, `σ = 0.20`). Mirrors the multi-scarp-cluster exposure pattern that v1.8's distance-to-nearest collapsed: a cell 25 km from one scarp scored identically to a cell 25 km from a *cluster* of five scarps, even though the cumulative shaking is 5× larger in the cluster case. The new kernel correctly accumulates that. Per-cell diagnostic siblings (`cum_pgv`, `nearest_scarp_distance_km`, `n_contributing_scarps`) persist alongside the score COG so downstream analysis can read the multi-scarp signal without rerunning the criterion. The legacy `criteria/seismic.py` module (distance-to-nearest logistic) stays in tree as the v1.8 alternative for sensitivity analysis.

  **v2.2 vs v2.1 catalog comparison (240 m, 70-site catalog): identical.** 70 / 70 v2.2 sites are within 10 m of a v2.1 site (zero rerank); selene-side match 52 / 70 = 74.3 % (unchanged); Wueller-side 42 / 73 = 57.5 % (unchanged); median matched-pair distance 1.87 km (unchanged). **Why the catalog doesn't shift**: the south-polar scarp density is high (median 98 contributing scarps within 250 km cutoff per selene-site cell, range [36, 137]), so cum_pgv is *uniformly* well above the single-event 0.45 anchor (median 2.44, range [1.4, 9.6]). The sigmoid saturates near 0 for every selene-site cell, making pgv_seismic's contribution to the within-polygon aggregate ranking effectively constant — sites pull from the same other-seven-criteria gradient the v2.1 ranking already used. The criterion *did* drop the aggregate mean (0.354 → 0.265) and per-region best/mean scores by ~0.10–0.18 (e.g. Mons Mouton best 0.541 → 0.437) because pgv_seismic now contributes ~0 instead of v1.8 seismic's ~0.95.

  **v2.2 vs v2.1 catalog comparison (20 m per-region tiled, 69-site Wueller-class catalog): essentially identical, +1 Wueller match.** v2.2 produces 69 sites across 8 of 9 NASA Artemis IV candidate regions (Malapert Massif still has zero HLS-compliant cells, identical to v1.5–v2.1); Wueller-side match **55 / 69 = 79.7 %** (was 54 / 69 = 78.3 % at v2.1, +1 site picked up the match), median matched-pair distance **1.85 km** (was 1.84 km). For comparison, v1.8 was 56 / 69 = 81.2 % at 1.69 km median; the 1-site drift between v1.8 and v2.x reflects the v2.0 EVA-disc and v2.1 multi-volatile geometry shifts, *not* the v2.2 PGV kernel — the kernel itself does not rerank within-polygon at 20 m for the same saturation reason as 240 m.

  **The substantive new content is the per-cell diagnostic surface, which v1.8's metric couldn't expose.** Per-cell diagnostics sampled at the 70 selene 240 m sites (data already on disk at `data/processed/scored/pgv_seismic_{cum_pgv,nearest_scarp_distance_km,n_contributing_scarps}_southpole_240m.tif`):

  | metric | min | P25 | median | P75 | max |
  |---|---:|---:|---:|---:|---:|
  | cum_pgv | 1.43 | 1.82 | 2.44 | 4.63 | 9.60 |
  | n_contributing_scarps | 36 | 39 | 98 | 115 | 137 |
  | nearest_scarp_distance (km) | 41.2 | 58.4 | 80.2 | 88.9 | 103.1 |

  Same diagnostic surface sampled at the **69 selene 20 m per-region tiled sites** (240 m diagnostic COG resampled to the 20 m site coordinates by nearest-cell lookup):

  | metric | min | median | mean | max |
  |---|---:|---:|---:|---:|
  | cum_pgv | 1.43 | 2.43 | 3.34 | 9.60 |
  | n_contributing_scarps | 36 | 98 | 84 | 136 |
  | nearest_scarp_distance (km) | 41.2 | 81.7 | — | 103.1 |

  At 20 m, **all 69 selene sites have ≥ 5 contributing scarps** (the spec's multi-scarp-cluster threshold) and **32 / 69 sites have ≥ 100 contributing scarps**, mirroring the 240 m pattern: the south-polar lobate-scarp field is *uniformly* dense across selene's HLS-compliant footprint, so v1.8's distance-to-nearest collapses every site to "≥ 41 km from the nearest scarp = safe" while the PGV kernel surfaces that nearly half the catalog sits in 100+-scarp clusters.

  **Multi-scarp-cluster exposure**: 70 / 70 selene sites have ≥ 20 scarps within 250 km cutoff (every site is in a multi-scarp environment); 49 / 70 have ≥ 50; **32 / 70 sites have ≥ 100 contributing scarps** (dense scarp regions that v1.8's distance-to-nearest would call "≥ 41 km from nearest scarp = safe"); 0 / 70 reach 150. The v1.8 logistic flagged sites within 5–25 km of a scarp as risky and everything beyond 50 km as safe — but every selene site is at least 41 km from the nearest scarp, so the v1.8 metric saturates at "safe" for the entire catalog. The PGV kernel surfaces that *half* the selene catalog sits in clusters of 100+ contributing scarps, with cumulative shaking 4–10× the single-strong-shaking-event reference.

  **Per-region cum_pgv medians** at the 240 m site catalog (lower = lower cumulative seismic exposure):

  | region | n | median cum_pgv | median n_contributing | median nearest scarp (km) |
  |---|---:|---:|---:|---:|
  | Slater Plain | 9 | 1.64 | 53 | 88.8 |
  | Mons Mouton Plateau | 10 | 1.73 | 120 | 88.9 |
  | Mons Mouton | 10 | 1.91 | 103 | 58.7 |
  | Nobile Rim 1 | 10 | 2.28 | 125 | 49.8 |
  | Haworth | 10 | 4.33 | 39 | 90.3 |
  | Peak Near Cabeus B | 9 | 4.76 | 36 | 86.9 |
  | Nobile Rim 2 | 10 | 5.04 | 108 | 66.1 |
  | de Gerlache Rim 2 | 2 | 9.40 | 39 | 55.7 |

  And at the **20 m per-region tiled site catalog** (same 240 m diagnostic COG nearest-cell sampled to the 20 m sites):

  | region | n | median cum_pgv | median n_contributing | median nearest scarp (km) |
  |---|---:|---:|---:|---:|
  | Mons Mouton Plateau | 10 | 1.60 | 118 | 91.2 |
  | Slater Plain | 8 | 1.65 | 54 | 87.1 |
  | Mons Mouton | 10 | 2.00 | 109 | 58.0 |
  | Nobile Rim 1 | 10 | 2.28 | 125 | 50.2 |
  | Haworth | 10 | 4.25 | 39 | 91.3 |
  | Peak Near Cabeus B | 9 | 4.76 | 36 | 86.9 |
  | Nobile Rim 2 | 10 | 5.05 | 108 | 66.2 |
  | de Gerlache Rim 2 | 2 | 9.40 | 39 | 55.7 |

  The 20 m per-region ordering matches the 240 m ordering exactly (Slater Plain / Mons Mouton Plateau lowest, de Gerlache Rim 2 highest), since the 240 m diagnostic surface is the source of truth for both — at 20 m the cells live inside the same 240 m parent voxel as their 240 m counterparts and inherit the same scarp-distribution context.

  **de Gerlache Rim 2** (already terrain-disagreeing with Wueller's catalog at 0 / 2 matched in v1.5–v2.1) has the highest median cum_pgv at 9.4 — its 2 surviving HLS-compliant cells sit in the densest cumulative-shaking environment of any selene region. **Slater Plain and Mons Mouton Plateau** have the lowest cumulative seismic exposure. The v1.8 metric's "≥ 41 km from nearest scarp" floor classified all eight regions identically; the PGV kernel separates them into a 5.7× spread (1.64 to 9.40) that the v1.8 distance-to-nearest collapsed.

  **Calibration note.** The default sigmoid midpoint (0.45) is anchored to a single Mw 5.3 source at 40 km — appropriate for sites in low-scarp-density regions but conservative for the dense south-polar lobate-scarp field where typical cum_pgv is 2–5. With this calibration the score saturates near 0 across the entire selene catalog, so the criterion's *catalog-level* effect is uniform-down-shift rather than within-polygon discrimination. **A data-driven calibration** (e.g. midpoint at the polar-grid median cum_pgv ≈ 2.9) would shift the criterion from a "low absolute floor" to a "relative-rank-within-polygon" signal — a candidate for v2.3+ as a separate scope, since the choice depends on whether the criterion is meant to discriminate *which* site within the polar polygon set is safest (relative) or *whether* the polar polygon set is safe vs other moon regions (absolute). v2.2 ships the absolute calibration the spec requested; per-cell diagnostics are independent of the sigmoid choice and surface the multi-scarp pattern regardless.

  **Activity-weighted PGV** (using Mishra & Kumar's 145 dated-scarp subset; weight by 1 / age) remains a v2.3+ candidate. v2.2 uses uniform activity probability 1.0, agnostic on the age-dating uncertainty.
...
- Watters, T. R. (2024). *Recent active thrust faulting on the Moon: implications for human exploration.* The Planetary Science Journal. (Source of the Mw ~5.3 strong-shaking PGV at 40 km from active scarps, used to calibrate v2.2's pgv_seismic kernel: `L = 50 km` decay, `cum_pgv ≈ 0.45 ≈ exp(-40 / 50)` sigmoid midpoint.)
...
- Lawrence, S. (2025). *Artemis IV Landing Site Process Overview.* NASA Lunar Site Selection Workshop, NTRS document 20250008952. [https://ntrs.nasa.gov/citations/20250008952](https://ntrs.nasa.gov/citations/20250008952). (Documents NASA's 5-step iterative site-selection process and the multi-directorate Figures of Merit (FOM) framework. Specific FOM weight values are not publicly disclosed; selene-base's "Mapping to NASA's Figures of Merit framework" subsection in Methodology positions the project's seven criteria against this published conceptual framework. Also cited as the framework reference for v2.0's EVA-disc PSR access criterion.)
...
- Paige, D. A., et al. (2010). *Diviner Lunar Radiometer observations of cold traps in the Moon's south polar region.* Science, 330(6003), 479–482. [doi:10.1126/science.1187726](https://doi.org/10.1126/science.1187726). (Source of the 110 K water-ice stability ceiling used as v2.0's `cold_threshold_k` default; cells whose annual maximum temperature stays below 110 K can hold water ice on geologic timescales.)
- Hayne, P. O., et al. (2015). *Evidence for exposed water ice in the Moon's south polar regions from LRO ultraviolet albedo and temperature measurements.* Icarus, 255, 58–69. [doi:10.1016/j.icarus.2015.03.032](https://doi.org/10.1016/j.icarus.2015.03.032). (Independent corroboration of the cold-class temperature regime that hosts surface water ice; secondary citation for v2.0's cold-class threshold.)
- Hayne, P. O., et al. (2020). *Micro cold traps on the Moon.* Nature Astronomy, 5(2), 169–175. [doi:10.1038/s41550-020-1198-9](https://doi.org/10.1038/s41550-020-1198-9). (Source of v2.1's three-class volatile thresholds — H₂O ≤ 110 K, CO₂/NH₃ ≤ 66 K, ultra-cold ≤ 60 K — for the deepest exotic-volatile traps. Documents the nested temperature regimes that host different volatile species on geologic timescales.)
