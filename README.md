...
# bypasses the auto-detect priority list (v1.8.3+):
...
- **v1.6 — 10 m exploratory analysis (deferred — three blockers identified).** ⏸ Three independent technical blockers were identified across multiple v1.6 attempts in May 2026:
  ...
  **(3) Memory ceiling at 10 m + 100 km buffer (architectural).** With v1.8.3's resolver fix and a clean re-download of the PGDA mosaic, a 3-region 10 m diagnostic was attempted on the cleanly-covered polygons (HW, SP, G2). The preprocess died ~46 seconds into the first tile from kernel OOM, taking the entire tmux session with it (not just the Python process). Per-tile dimensions at 10 m + 100 km buffer are ~22000×22000 = **484 M pixels**; the per-pixel horizon profile output array (36 azimuths × float32) is **~70 GB per tile** before scratch buffers and CuPy memory-pool overhead. v1.5's 20 m tiles were ~11000×11000 = 121 M pixels with a ~17 GB output array — comfortable in the GB10's 119 GB unified memory. At 10 m on the same buffer scope, the in-memory footprint is ~4× larger and exceeds the unified-memory cap; 5 m would be 16× larger and even further out of reach. Resolution requires architectural change: tile-internal chunking with buffer overlap and stitched output (each polygon-tile split into ~16 sub-tiles of ~5500×5500 pixels with ~10000-pixel buffer overlap, processed sequentially on GPU with explicit mempool drains between sub-tiles, then stitched back). Tractable but non-trivial; targeted as v1.9 / v2.0 scope. See "Where this goes next" below.

  Until tile-internal chunking lands, v1.6 (and any 5 m work at the candidate-region geographic extent) is structurally blocked. The 10 m mosaic file remains on disk (`data/raw/lola/ldem_83s_10mpp_adj.tif`, 4.77 GB, full-pixel-data checksum 41127) for follow-up after the architecture work.
...
**Tile-internal chunking architecture for ≤ 10 m analysis (the second v1.6 blocker).** Even on a polygon that *does* fit inside a finer mosaic, the per-region tiled pipeline as written hits the GB10's 119 GB unified-memory cap at 10 m + 100 km buffer (per-tile output array ~70 GB, before scratch buffers and CuPy memory-pool overhead). The OOM killer takes the entire tmux session, not just the Python process. Resolving requires splitting each polygon-tile into ~16 sub-tiles of ~5500×5500 pixels with ~10000-pixel buffer overlap, processing on GPU sequentially with explicit mempool drains between sub-tiles, and stitching the per-criterion outputs back. Architectural change of ~half-day-to-day scope; would unblock v1.6 (10 m on the 3 cleanly-covered polygons) and is a prerequisite for any 5 m work that becomes data-feasible (NAC stereo DEM mosaics, future higher-latitude PGDA products). Targeted as v1.9 / v2.0 scope, no external dependency.
